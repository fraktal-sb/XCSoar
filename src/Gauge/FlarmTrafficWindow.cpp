                                     ...
                                     unsigned i) noexcept
{
  ...
  if (traffic.HasName() && (side_display_type == SideInfoType::RELATIVE_ALTITUDE)) {
    ...
    tstring_view text = (len > 3)
        ? tstring_view(traffic.name.c_str() + (len - 2), 2)
        : tstring_view(traffic.name);
  ...
  if (side_display_type == SideInfoType::VARIO) {
    ...
    const short relalt = iround(Units::ToUserAltitude(traffic.relative_altitude) / 100
    );
    _stprintf(txtbuf, _T("   %+d"), static_cast<int>(relalt));
  }

  const PixelPoint tp{
    sc[i].x + int(sx.height / 2),
