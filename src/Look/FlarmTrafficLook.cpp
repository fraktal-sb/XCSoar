...
void
FlarmTrafficLook::InitialisePensAndFonts() noexcept
{
  const unsigned width = Layout::ScalePenWidth(small ? 1u : 2u);
  ...
  call_sign_font.Load(FontDescription(Layout::FontScale(22), true));
}

void
FlarmTrafficLook::Initialise(const TrafficLook &other, bool _small, bool _inverse)
{
  small = _small;
  inverse = _inverse;
  ...
  InitialisePensAndFonts();
}

void
FlarmTrafficLook::ReinitialiseLayout() noexcept
{
  InitialisePensAndFonts();
}
